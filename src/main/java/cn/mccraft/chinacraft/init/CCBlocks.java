...
//    BlockCrusher IRON_CRUSHER = new BlockCrusher(EnumCrusherMaterial.IRON);
    ...
    Block BAMBOO_FENCE_GATE = new BlockCCFenceGate(Material.WOOD).setSoundType(SoundType.WOOD).setHardness(2.0F).setResistance(5.0F);

    @RegBlock(value = {"bamboo", "door"}, itemClass = ItemCCDoor.class)
    Block BAMBOO_DOOR = new BlockCCDoor(Material.WOOD).setHarvestLevelReturnBlock("axe", 0).setSoundType(SoundType.WOOD).setHardness(1.0F).setResistance(2.0F);
